...
def verify_token(token_value: str, timeout: int = 15) -> Optional[str]:
    """
...
def get_account_id(token_value: str, timeout: int = 15) -> Optional[str]:
    """
...
def list_buckets(token_value: str, account_id: str, timeout: int = 15) -> list[str]:
    """
    列出账户下的 R2 桶名。
...
def create_bucket(token_value: str, account_id: str, name: str, timeout: int = 15) -> bool:
    """
...
def auto_configure(token_value: str, prefer_bucket: str = "", create_if_missing: bool = False,
    ...
    access_key_id = verify_token(token_value)
    ...
    account_id = get_account_id(token_value)
    if not account_id:
