...
Discovery uses the public Guangying movie API, then object writes use Wrangler.
Set CLOUDFLARE_API_TOKEN in the environment before running.
"""
...
def movie_slugs(page_size: int, max_pages: int):
    page = 1
    while True:
        if max_pages and page > max_pages:
            break
        data = fetch_json(f"{SITE_URL}/api/movies?page={page}&pageSize={page_size}")
...
def discover_r2_dirs(page_size: int, max_pages: int) -> list[str]:
    dirs: set[str] = set()
    for slug in movie_slugs(page_size, max_pages):
        try:
            detail = fetch_json(f"{SITE_URL}/api/movies/{quote(slug)}")
...
def build_subtitle_tracks(r2_dir: str, subtitles: list[dict], workdir: Path) -> list[dict]:
    used: dict[str, int] = {}
    tracks: list[dict] = []
    for index, sub in enumerate(subtitles):
        ...
        if not url:
            continue
        text = normalize_vtt(fetch_text(urljoin(SITE_URL, url)))
...
def backfill_dir(r2_dir: str, bucket: str, env: dict[str, str], dry_run: bool, force: bool) -> str:
    subtitles_url = f"{SITE_URL}/api/r2/subtitles/{r2_dir}"
    subtitles = fetch_json(subtitles_url)
    if not subtitles:
        return "no_subtitles"
    ...
    with tempfile.TemporaryDirectory(prefix="gy-subtitles-") as tmp:
        ...
        tracks = build_subtitle_tracks(r2_dir, subtitles, workdir)
        if not tracks:
...
def main() -> int:
    ...
    env = dict(os.environ)
    dirs = sorted(set(args.dirs)) if args.dirs else discover_r2_dirs(args.page_size, args.max_pages)
    ...
    for i, r2_dir in enumerate(dirs, 1):
        ...
        try:
            status = backfill_dir(r2_dir, args.bucket, env, args.dry_run, args.force)
